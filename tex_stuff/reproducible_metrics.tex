...
The rough idea for the econometric analysis in the paper is adapted from an independent project I completed during my Junior year. In a subsequent class the project was modified and rebuilt in a reproducible fashion. Sarah Johnson created the original intermediate functions in \inlinecode{p3functions.py} \href{https://github.com/nadavtadelis/Reproducible_Metrics/blob/master/p3functions.py}{\textcolor{cyan}{\inlinecode{p3functions.py}}}, and the associated tests and Travis integration. Chitwan Kaudan created the original Makefile for running the individual Jupyter Notebooks. All aspects of those original analyses have been altered significantly, and the history of the alterations is fully documented in the commit history of the git repo.
...
\newpage
\section{Models}
We need to first set up our structural equation \textcolor{BrickRed}{[Maybe this isn't actually a structural equation? Need to double check this terminology]} defining the relationship between grades and studying (\cite{CardKrueger}). Let an individual's grade be $g_i$ and weekly hours of studying be $s_i$ and their ``ability" be $a_i$. Then our model is:
$$
g_i = \beta_0 + \beta_1 s_i + \beta_2 s_i^2  + \beta_3 a_i + \boldsymbol{\beta}_{4:k}X_i + \varepsilon_i
$$
...
\textcolor{BrickRed}{[Lasso-flavor penalization and VIF discussion here (maybe no numbers, just general take-aways and link to the relevant notebook), point out that Lasso keeps the variable of interest and also explore the other variables that it weights highly]} \\
note: in lasso section cite: (\cite{MLmetrics})  \\
note: in VIF section cite: (\cite{VIF}) and (\cite{detecting}) \\
Make sure to include discussion of why it seems okay to include parental education even though it is a bit collinear - because they are considered good proxies for ability
...
$$
Where $x_{i,5}, ..., x_{i,k}$ are proxies and $\nu_i$ is the structural error term. Let $x_{i,j}$ be the j'th covariate in the design matrix ($x_{1 i} = 1$, $x_{2 i} = s_i$, etc.) and let $\bm{x}_i$ be the vector of covariates for $i$. We are mainly interested in $\beta_1$ and $\beta_2$ the coefficients on studying. It is reasonable to assume that $\E[\nu_i | \bm{x}_i, q_i] = 0$ unfortunately we have no choice but to stick $q_i$ into the error term which gives us the new structural equation:
$$
...
%~~~~~~~~~~~~~~~~~~
\subsection{Q2SLS}
\textcolor{BrickRed}{[Introduce idea of instrumental variables really briefly, maybe just go over the conditions for validity of instruments (see page 45 of 212 notes)]}
...
\begin{enumerate}[label={(\arabic*)}]
	...
	\item In the reduced form of the endogenous variable $x_{i,j} = \phi_0 x_{i,1} + \phi_1 school_i + \cdots + \phi_{k-2}x_{i,k} + \theta_1 z_i + \vartheta_i$ the coefficient on our instrument is nonzero: $\theta_1 \ne 0$
...
In section \ref{endogeneity} we established that the causal relationship between our grades and studying time is quadratic. \textcolor{BrickRed}{[NOTE: for now I'm going to assume we only end up using the 'good' binary instrument, but even if we end up using more binary instruments, the procedure still holds but it is less clear that its the right approach.]} This would usually not be a barrier to using 2SLS; the standard approach\footnote{Assuming only one instrument is available.} is to use the original instrument and its square as instruments in the two first stage equations (\cite{harmless}). However, this approach is unusable when the only instrument available is binary. In fact, even if we have multiple binary instruments it is not convincing to think that the exogenous portion of the squared endogenous variable will be correctly identified because interacting the binary instruments gives very little additional variation \textcolor{BrickRed}{[THINK ABOUT THIS MORE]}. 

%~~~~~~~~~~~~~~~~~~
\subsubsection{Estimation Procedure}
\textcolor{BrickRed}{[describe how to for q2sls]} or maybe this should be in above section?
...
\textcolor{BrickRed}{[Give brief overview of bootstrapping and explain how its being used here to estimate variance of coeff. estimates in 2nd stage]} Also add note about future integration of a wild bootstrap approach to variance estimation (stricter assumptions, but some really nice properties for endogenous models - see slide 15 from the link below)

\textcolor{BrickRed}{FOR BOOTSTRAPPING COV MATRIX:} See 8th slide of \textcolor{cyan}{https://www.math.kth.se/matstat/gru/sf2930/papers/wild.bootstrap.pdf}. It also has notes on hypothesis testing and p values for bootstrapping with regression coefficients. SLIDE 14 HAS DISCUSSION OF BOOTSTRAPPING WITH ENDOGENOUS VARS!!
