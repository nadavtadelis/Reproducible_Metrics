...
This paper was written as my honors thesis for undergraduate studies in statistics at UC Berkeley. I would like to thank my thesis advisor, Professor Fernando P\'erez, for his support and insights. Due to deadlines for submission I was unable to spend an appropriate amount of time on the econometric analysis, and will point out some of the weak points in my models (specifically the instruments). Any comments or suggestions would be greatly appreciated. 
...
\textcolor{BrickRed}{[Plots need finishing (better labels, axis names, titles for with/without controls, choose line type and color, etc.]} when explaining the plots, maybe call the y axis study time's contribution to the predicted G3 percent? Currently the leftmost plots are from the models without individual level controls, the right plots are from models with controls (maybe call them proxies here instead of controls because we call point out later that they are proxies?)
...
note: in VIF section cite: (\cite{VIF, detecting}) \\
Include discussion of why it seems okay to include parental education even though it is a bit collinear - because they are considered good proxies for ability

%~~~~~~~~~~~~~~~~~~
\subsubsection{Simultaneous Causality}
\textcolor{BrickRed}{[Need to edit this section, right now its messy and not detailed enough]}

\textcolor{BrickRed}{[Maybe move this to right before the Naive OLS section? Or move this to the Data Issues as its own subsection?]}
