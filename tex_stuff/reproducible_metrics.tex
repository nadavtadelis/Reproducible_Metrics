...
In the model fitting 1 notebook we also include some additional explorations of the estimated models. We compute the variance inflation factor to check for multicollinearity. We find that the most collinear variables are parental education with VIF's of 20-30. While this might normally suggest that we omit these variables (or use their principal components to get isolate the sources of variance), in this case we actually want to retain these slightly collinear variables (\cite{VIF, detecting}). We keep parental education because it has historically been considered an exceptionally good proxy for ability, and we can assume that its contribution to controlling for ability outweighs any multicollinearity it contributes. We also find slight collinearity between studying time and its square, but we are less interested in this collinearity because it is on our variable of interest, and the VIF is low enough to ignore.
